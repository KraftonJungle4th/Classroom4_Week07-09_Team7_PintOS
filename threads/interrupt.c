...
/* External interrupts are those generated by devices outside the
   ...
   외부 인터럽트는 타이머와 같은 CPU 외부 장치에서 생성됩니다.
   외부 인터럽트는 인터럽트가 꺼진 상태에서 실행되므로 중첩되지 않으며 언제나 선점되지 않습니다.
   외부 인터럽트의 핸들러는 잠들 수 없지만,
   인터럽트가 반환되기 직전에 새 프로세스가 스케쥴되도록 요청하기 위해 intr_yield_on_return()을 호출할 수 있습니다.
   */
...
/* Registers external interrupt VEC_NO to invoke HANDLER, which
   ...
   execute with interrupts disabled.
	...
	디버깅 목적으로 이름이 지정된 HANDLER를 호출하도록 외부 인터럽트 VEC_NO를 등록합니다.
...
/* During processing of an external interrupt, directs the
   ...
   외부 인터럽트 처리 중에는 인터럽트 핸들러가 인터럽트에서 반환하기 직전에 새 프로세스에 양보하도록 지시합니다.
   다른 시간에는 호출할 수 없습니다. */
